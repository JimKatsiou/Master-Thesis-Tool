...
%% Generate Json files
    if (sc == 1)  % 5g scenario
	    ...
	    movefile('GA-cost-effective-5g-solutions.json','Results\GA')
    elseif (sc == 2)
        % cost-effective (cheapest) 5g solution (sort-by-solution)
        %s  = table2struct(S_population);
	    jsonText = jsonencode(S_population); % Convert to JSON text
	    ...
	    movefile('GA-cost-effective-lora-solutions.json','Results\GA')
    elseif (sc == 3)
        % cost-effective (cheapest) 5g solution (sort-by-solution)
        %s  = table2struct(S_population);
	    jsonText = jsonencode(S_population); % Convert to JSON text
	    ...
	    movefile('GA-cost-effective-NB-solutions.json','Results\GA') 
    end
