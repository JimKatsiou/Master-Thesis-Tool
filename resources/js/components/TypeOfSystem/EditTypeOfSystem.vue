<template>
    <div class="topOfThePage">
        <h3>Edit Type of System</h3>
    </div>
    <div class="nextOfSideBar">
        <div class="row">
            <div class="col-md-6" v-if="type_of_system">
                <form @submit.prevent="updateType_of_system" v-if="type_of_system">
...
export default {
    ...
    created() {
        this.$axios.get('/sanctum/csrf-cookie').then(response => {
            this.$axios.get(`/api/type_of_systems/edit/${this.$route.params.id}`)
                .then(response => {
                    this.type_of_system = response.data;
                })
    ...
    methods: {
        updateType_of_system() {
            this.$axios.get('/sanctum/csrf-cookie').then(response => {
                this.$axios.post(`/api/type_of_systems/update/${this.$route.params.id}`, this.type_of_system)
                    .then(response => {
